...
var (
	birthdayDBTypes = map[string]string{`ID`: `INTEGER`, `UserID`: `INTEGER`, `Name`: `TEXT`, `Date`: `DATE`, `CreatedAt`: `DATETIME`, `UpdatedAt`: `DATETIME`}
	_               = bytes.MinRead
)
...
func testBirthdaysSliceUpdateAll(t *testing.T) {
	...
	if strmangle.StringSliceMatch(birthdayAllColumns, birthdayPrimaryKeyColumns) {
		...
		fields = strmangle.SetComplement(fields, birthdayGeneratedColumns)
	}
