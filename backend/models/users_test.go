...
func testUserToManyBirthdays(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = randomize.Struct(seed, &b, birthdayDBTypes, false, birthdayColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	if err = randomize.Struct(seed, &c, birthdayDBTypes, false, birthdayColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	...
	check, err := a.Birthdays().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	slice := UserSlice{&a}
	if err = a.L.LoadBirthdays(ctx, tx, false, (*[]*User)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.Birthdays); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.Birthdays = nil
	if err = a.L.LoadBirthdays(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.Birthdays); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testUserToManyAddOpBirthdays(t *testing.T) {
	var err error
	...
	foreignersSplitByInsertion := [][]*Birthday{
		{&b, &c},
		{&d, &e},
	}

	for i, x := range foreignersSplitByInsertion {
		err = a.AddBirthdays(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.R.Birthdays[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.Birthdays[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}
...
var (
	userDBTypes = map[string]string{`ID`: `INTEGER`, `EmailHash`: `TEXT`, `PasswordHash`: `TEXT`, `ReminderTime`: `TEXT`, `Timezone`: `TEXT`, `TelegramBotAPIKey`: `TEXT`, `TelegramBotAPIKeyHash`: `TEXT`, `TelegramUserID`: `TEXT`, `TelegramUserIDHash`: `TEXT`, `CreatedAt`: `DATETIME`, `UpdatedAt`: `DATETIME`}
	_           = bytes.MinRead
)
