...
var BirthdayWhere = struct {
	...
	ID:        whereHelpernull_Int64{field: "\"birthdays\".\"id\""},
	UserID:    whereHelperint64{field: "\"birthdays\".\"user_id\""},
...
var (
	...
	birthdayGeneratedColumns      = []string{"id"}
)
...
func FindBirthday(ctx context.Context, exec boil.ContextExecutor, iD null.Int64, selectCols ...string) (*Birthday, error) {
	...
	query := fmt.Sprintf(
		"select %s from \"birthdays\" where \"id\"=?", sel,
	)
...
func (o *Birthday) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if !cached {
		...
		wl = strmangle.SetComplement(wl, birthdayGeneratedColumns)
...
func (o BirthdaySlice) UpdateAll(ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	sql := fmt.Sprintf("UPDATE \"birthdays\" SET %s WHERE %s",
		...
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 0, birthdayPrimaryKeyColumns, len(o)))
...
func (o *Birthday) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns) error {
	if o == nil {
	...
	if !cached {
		...
		cache.query = buildUpsertQuerySQLite(dialect, "\"birthdays\"", updateOnConflict, ret, update, conflict, insert)
...
func BirthdayExists(ctx context.Context, exec boil.ContextExecutor, iD null.Int64) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from \"birthdays\" where \"id\"=? limit 1)"
